...
fn main() {
    ...
    for i in 0..lines.len()-1 {
        for j in i..lines.len() {
            let mut number: Vec<String> = lines[i].chars().map(|x| x.to_string()).collect();
            let mut new_number = vec![String::from("[")];
            new_number.append(&mut number);
            new_number.push(String::from(","));
            new_number.append(&mut lines[j].chars().map(|x| x.to_string()).collect::<Vec<String>>());
            new_number.push(String::from("]"));
            number = new_number;
            loop {
            ...
            let mag = magnitude(&mut number);
            if mag > max { max = mag }
        }
    }
    for i in 0..lines.len()-1 {
        for j in i..lines.len() {
            let mut number: Vec<String> = lines[j].chars().map(|x| x.to_string()).collect();
            let mut new_number = vec![String::from("[")];
            new_number.append(&mut number);
            new_number.push(String::from(","));
            new_number.append(&mut lines[i].chars().map(|x| x.to_string()).collect::<Vec<String>>());
            new_number.push(String::from("]"));
            number = new_number;
            loop {
                if explode(&mut number) {
