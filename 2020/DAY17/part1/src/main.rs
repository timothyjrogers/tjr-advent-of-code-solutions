...
fn get_neighbors(point: (i32, i32, i32)) -> Vec<(i32,i32,i32)> {
    let mut neighbors: Vec<(i32,i32,i32)> = vec![];
    for i in -1..=1 {
        for j in -1..=1 {
            for k in -1..=1 {
                let new_point = (point.0 + i, point.1 + j, point.2 + k);
...
fn main() {
    ...
    let mut active_points:  HashSet<(i32,i32,i32)> = HashSet::new();
    for (y, v) in vec.iter().enumerate() {
        for (x, c) in v.chars().enumerate() {
            if c == '#' {
                active_points.insert((x as i32, y as i32, 0));
            }
        }
    }

    for generation in 0..6 {
        let mut new_active_points: HashSet<(i32,i32,i32)> = HashSet::new();
        let mut inactive_points: HashSet<(i32,i32,i32)> = HashSet::new();
